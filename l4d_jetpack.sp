...
new bool:CheckEngineer[MAXPLAYERS + 1] = false;
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_jetpack", SpawnJetPack);
...
public Action:player_use(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	if(!CheckEngineer[client])return;
...
public ClientTakeEngineer(client, args) 
{
	CheckEngineer[client] = true;
}
public ClientDropEngineer(client, args) 
{
	CheckEngineer[client] = false;
	if(HaveJetPack[client]!=false)RemoveJetPack(client, true);
}
public Action:SpawnJetPack(client, args)
{
	if (client == 0 || !IsPlayerAlive(client) || GetClientTeam(client) != 2)
		return;
	if (CheckEngineer[client])
		sm_jetpack(client, args);
	else PrintToChat(client, "%t", "only_engineers");
}
...
public Action:sm_jetpack(client,args)
{
	if(client>0 && IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		else
		{
			...
			else PrintToChat(client, "Вы ввели неверный пароль.");
		}
	}
	return Plugin_Continue;
}
...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	if(CheckEngineer[victim] && HaveJetPack[victim])
	{
		...
		{ 
			...
			{ 
				...
				if(0.0 < fDistance <= 100.0) SDKHooks_TakeDamage(i, victim, victim, fDistance, DMG_BURN);//Урон, это fDistance, то есть в данном примере урон равен дистанции от взрыва до игрока.
				else if (fDistance == 0.0) SDKHooks_TakeDamage(i, victim, victim, 50.0, DMG_BURN);
			} 
		}
	}
	if (CheckEngineer[victim]) CheckEngineer[victim] = false;
...
DropJetpack(Float:pos[3], flag)
{
	...
	DropCount++;
	if(flag==0)PrintToChatAll("Реактивный ранец изъят у танка.");
	if(flag==1)PrintToChatAll("Реактивный ранец изъят у ведьмы");
}
...
{ 
	...
	DispatchKeyValue(button, "use_string", "Надеваю...");
...
public OnPressed(const String:output[], caller, activator, Float:delay)
{
	...
	if(!CheckEngineer[activator])
	{
		...
		{ 
			...
			{ 
				GetClientAbsOrigin(i, fClientPos); 
				fDistance = GetVectorDistance(fClientPos, fPos);
				if(0.0 < fDistance <= 100.0) SDKHooks_TakeDamage(i, activator, activator, fDistance, DMG_BURN);//Урон, это fDistance, то есть в данном примере урон равен дистанции от взрыва до игрока.
	...
	if(find >=0 && CheckEngineer[activator])
	{
...
public bool:Get_PosFilter(ent, mask, any:i) return i != ent;*/