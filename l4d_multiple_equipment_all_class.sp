...
//new bool:CheckRecon[MAXPLAYERS+1];
new bool:CheckEngineer[MAXPLAYERS+1];
new bool:CheckSupport[MAXPLAYERS+1];
//new bool:CheckMedic[MAXPLAYERS+1];
new bool:CountUse[MAXPLAYERS+1];
new bool:PlayersMove;
...
Process(client, Float:time, button, bool:isSwitch, currentWeapon=0)
{
	...
	for(new slot=0; slot<5; slot++)
	{
		...
		if(GetConVarInt(l4d_me_slot[slot])==0 || CheckSupport[client])continue;
		if( slot==2 )
		{
			if (CheckEngineer[client] == false)continue;
...
LoadEquipment(i)
{
	...
	decl item;
	if (IsClientInGame(i) && (item = GetPlayerWeaponSlot(i, 0)) > 0)
	{
		decl String:weapon[25];
		GetEdictClassname(item, weapon, sizeof(weapon));
		if (StrEqual(weapon, "weapon_hunting_rifle") || StrEqual(weapon, "weapon_autoshotgun"))
		{
			if(StrEqual(ItemName[i][0], "weapon_hunting_rifle") || StrEqual(ItemName[i][0], "weapon_autoshotgun"))
			{
				DropSecondaryPrimaryWeapons(i);
			}
		}
	}
}
...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	if(client>0 && IsClientInGame(client) && GetClientTeam(client)==2)
	{
		RemoveItmeAttach(client,-1);
		DropSecondaryItem(client);
		//if (CheckRecon[client]) CheckRecon[client] = false;
		if (CheckEngineer[client]) CheckEngineer[client] = false;
		if (CheckSupport[client]) CheckSupport[client] = false;
		//if (CheckMedic[client]) CheckMedic[client] = false;
	}
	ResetClientState(client);
}
...
public OnClientPutInServer(client)
{
	...
	FirstRun[client]=true;
	MeEnable[client]=false;
	if (b_FirstMap) ResetClientState(client);
	CreateTimer(1.0, Timer_EnableClient, client);
}
...
public Action:round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new i=1; i<=MaxClients; i++)
	{
		if (CheckSupport[i]) CheckSupport[i] = false;
		//if (CheckMedic[i]) CheckMedic[i] = false;
		if (CheckEngineer[i]) CheckEngineer[i] = false;
	}
}
/*public Action:finale_win(Handle:event, const String:name[], bool:dontBroadcast)
{
	UnHookAll();
	DisableAllClient();
	DisableAllFirstRun();
	PrintToServer("[Equipment] Finale Win");
	for(new i=1; i<=MaxClients; i++)
	{
		RemoveItmeAttach(i,-1);
		ResetClientState(i);
		if (CheckSupport[i]) CheckSupport[i] = false;
		//if (CheckMedic[i]) CheckMedic[i] = false;
		if (CheckEngineer[i]) CheckEngineer[i] = false;
...
{
	PlayersMove = true;
	//PrintToServer("PlayersMove = TRUE!");
}
...
/*public ClientTakeRecon(client, args) 
{
	CheckRecon[client] = true;
}*/
...
public ClientTakeSupport(client, args) 
{
	...
	DropSecondaryPrimaryWeapons(client);
}
/*public ClientTakeMedic(client, args) 
{
	CheckMedic[client] = true;
	...
	if ((item = GetPlayerWeaponSlot(client, 0)) > 0 && IsClientInGame(client))
	{
		...
		if (StrEqual(weapon, "weapon_hunting_rifle") || StrEqual(weapon, "weapon_autoshotgun"))
		{
			new Float:origin[3];
			GetClientEyePosition(client,origin);
			if(StrEqual(ItemName[client][0], "weapon_hunting_rifle") || StrEqual(ItemName[client][0], "weapon_autoshotgun"))
			{
				DropSecondaryPrimaryWeapons(client);
			}
		}
...
/*public ClientDropRecon(client, args) 
{
	DropSecondaryHuntingRifle(client);
	
	new oldweapon=0;
	decl String:oldWeaponName[LEN64]="";
	ItemName[client][0]=oldWeaponName;
	GetItemClass(oldweapon, oldWeaponName);
	CheckRecon[client] = false;
}
DropSecondaryHuntingRifle(client)
{
	new Float:origin[3];
	...
	{
		new ent=CreateWeaponEnt(ItemName[client][0]);
		TeleportEntity(ent, origin, NULL_VECTOR, NULL_VECTOR);
		ActivateEntity(ent);
		RemoveItmeAttach(client, 0);
		ItemAttachEnt[client][0]=0;
	}
}*/
public ClientDropEngineer(client, args) 
{
	DropSecondaryGranade(client);
	
	new oldweapon=0;
	decl String:oldWeaponName[LEN64]="";
	ItemName[client][2]=oldWeaponName;
	GetItemClass(oldweapon, oldWeaponName);
	
	CheckEngineer[client] = false;
	RemoveItmeAttach(client, 2);
	ItemAttachEnt[client][2]=0;
}
DropSecondaryGranade(client)
{
	if (IsClientInGame(client))
	{
		new Float:origin[3];
		GetClientEyePosition(client,origin);
		if(!StrEqual(ItemName[client][2], ""))
		{
			new ent=CreateWeaponEnt(ItemName[client][2]);
			TeleportEntity(ent, origin, NULL_VECTOR, NULL_VECTOR);
			ActivateEntity(ent);
		}
	}
...
/*public ClientDropMedic(client, args) 
{
	CheckMedic[client] = false;
}*/
DropSecondaryPrimaryWeapons(client)
{
	new Float:origin[3];
	GetClientEyePosition(client,origin);
	if(!StrEqual(ItemName[client][0], ""))
	{
		new ent=CreateWeaponEnt(ItemName[client][0]);
		TeleportEntity(ent, origin, NULL_VECTOR, NULL_VECTOR);
		ActivateEntity(ent);
		...
		ItemAttachEnt[client][0]=0;
		ItemName[client][0]="";
		ItemInfo[client][0][0]=0;
		ItemInfo[client][0][1]=0;
		ItemInfo[client][0][2]=0;
		ItemInfo[client][0][3]=0;
	}
}
...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	if(StrContains(iWeaponName, "weapon_hunting_rifle", false) != -1 || StrContains(iWeaponName, "weapon_autoshotgun", false) != -1)
	{
		if(IsFakeClient(client) && PlayersMove)
		{
			return Plugin_Handled;
		}
		if(StrEqual(ItemName[client][0], "weapon_hunting_rifle") || StrEqual(ItemName[client][0], "weapon_autoshotgun"))
		{
			if (!CountUse[client])
			{
				if (StrContains(iWeaponName, "weapon_autoshotgun", false) != -1) PrintToChat(client, "%t", "HaveHuntingRifle");
				else PrintToChat(client, "%t", "HaveAutoshotgun");
				CountUse[client] = true;
				CreateTimer(5.0, Timer_AnonceHave, client);
			}
			return Plugin_Handled;
...
g_bFirstMap()
{
	...
	StrEqual(mapname, "l4d_sh01_oldsh")) b_FirstMap = true;
	else b_FirstMap = false;
}
